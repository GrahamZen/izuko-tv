...
记忆源是本番剧最近一次真正播起来的 Web 源（自动选中的也算）或用户手动选的 Web 源，
由 `ObserveWebMediaSourcePreferenceExtension` 维护；记忆源查询失败时被删除。
偏好 BT 时只记手动选的：自动选中 Web 源多半是 BT 这集没有资源时的兜底，记下来会让之后越过 BT 直接选它。
开始自动选择时如果有数据源处于暂停状态（开播后暂停的，切到下一集或换源时会遇到，见下文），
记忆源给不出可选资源、进入下面的截止时间阶段时，自动选择把它们放开重新查询。
...
模糊匹配仍需等待第二截止时间。播放失败换源仍只尝试 Web：手上还有其他 Web 候选时两个截止时间均为 1 秒，
并在最后阶段无结果时返回；已经没有候选、但还有 Web 源没查完（包括被暂停、此时放开重查的）时，
按默认的两个截止时间等待它们的结果。两种情况都保留黑名单且不覆盖等待期间的手动选择。

视频真正开始播放（播放器时钟在走）之前，所有数据源与原来一样同时查询。开始播放后按下面的规则暂停与放开。
「开始播放」只认播放状态从没在播变成在播的那一刻：换了查询会话（如重新搜索）时视频已经在播，不算开始播放。

| 时机 | 行为 |
| --- | --- |
| 开始播放，选源面板关着、没开「完整搜索」 | `PauseMediaFetchWhilePlayingExtension` 暂停仍在查询的数据源（本地缓存除外），已拿到的结果保留 |
| 打开选源面板 | 放开被暂停的数据源；面板开着期间不再暂停（换源后新资源开播、缓冲恢复都不暂停） |
| 关闭选源面板 | 开了「完整搜索」或没在播放：照常查；正在播放：暂停还没查完的数据源 |
| 打开「完整搜索」（面板顶部的开关、手机控制中心的按钮） | 放开被暂停的数据源，本播放页之后一直查完，不再暂停 |
| 用户主动重新搜索（选源面板的刷新、手机控制中心的「重新搜索（含新数据源）」） | 同打开「完整搜索」：新一轮一直查完 |
| 用户暂停播放、缓冲 | 不放开（否则每次都从头重查），再播起来时照常判断 |
| 播放失败换源、切到下一集 | 见上文：手上的结果不够用时放开 |

「放开」是从头重新查询，不接着暂停前的进度。选源面板不单独标出被暂停的数据源；
面板里没拿到结果、被暂停的 Web 源与查询成功但 0 条的一样不显示。手机控制中心没有“打开面板”的时机，
有数据源被暂停时在「重新搜索」旁边显示「完整搜索」。
