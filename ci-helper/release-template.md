...
- 电视详情页改成按页翻：海报 → 选集 → 角色和制作人员 → 评价和关联条目，每按一次下键正好停在下一页的开头。角色与制作人员改成圆头像横排，聚焦时放大；评价改成一行评论卡，点哪条就进哪条（末尾一张卡进全部评价）。原来的「作品信息」块去掉了（放送时间和话数首屏已有，别名移到选集页简介的「显示更多」里）
- 进详情页途中按返回不再要等一秒；放大进来的详情页按返回会缩回原来的卡片
