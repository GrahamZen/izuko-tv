...
本仓库 (Izuko TV) 是 [open-ani/animeko](https://github.com/open-ani/animeko) 的 Android TV fork, 依照 AGPL-3.0 发布。
...
`src/izukoTv` 给 `ui-tv` 里的 `TvAniUiBehavior`。
...
注入点唯一:`app/android/src/izukoTv/kotlin/tv/TvPageVariants.kt`。
...
| `app/android/src/izukoTv/AndroidManifest.xml` | leanback 入口 / banner / EPG 权限 / 屏保 service |
| `app/android/src/izukoTv/kotlin/FormFactorSetup.kt` | tv 变体接缝: 行为开关 + 装插槽 + 主屏频道初始化 |
| `app/android/src/phone/kotlin/FormFactorSetup.kt` | phone 变体接缝 (全部 no-op) |
| `app/android/src/izukoTv/kotlin/tv/TvPageVariants.kt` | 把 TV 页面塞进 6 个变体插槽 |
| `app/android/src/izukoTv/kotlin/tv/AniDreamService.kt` | TV 屏保 (Daydream) |
| `app/android/src/izukoTv/kotlin/tv/TvHomeChannels.kt` | 主屏预览频道 / 继续观看 |
| `app/android/src/izukoTv/res/drawable/tv_banner.xml` | 主屏图标 banner |
...
| 文件 | 用途 |
|---|---|
| `app-lang .../res/values*/strings_izuko.xml` (4 份) | fork 新增的文案; 新 key 一律写这里, 不写 `strings.xml` |
| `scripts/fork-rebase.sh` `scripts/rebase/` | 追上游的流程脚本与冲突自动规则, 见第七节 |
...
| `app-lang` / `app/shared` `strings.xml` | 改写部分上游文案的措辞 (`app/shared` 那份另有远程保活通知两条新增); `app-lang` 的新增文案都在同目录 `strings_izuko.xml`。rebase 时按 key 合并, 见第七节 |
...
用 `scripts/fork-rebase.sh` 一趟重放整栈 (各子命令的注意事项写在脚本文件头):

```
./scripts/fork-rebase.sh preflight   # 这次会在哪打架, 不动任何东西
./scripts/fork-rebase.sh dryrun      # 内存里整栈重放, 报告自动规则处理完还剩几处要人手 (不动任何东西)
./scripts/fork-rebase.sh run         # 打备份 tag + 重放; 停下时先自动处理机械冲突
./scripts/fork-rebase.sh continue    # 解完要人手的那处、git add 之后接着走
./scripts/fork-rebase.sh verify      # range-diff 逐条对照重放前后
```

自动处理的只有结果唯一的几类冲突, 其余照旧停下来给人:

- 目录改名 (如上游把 `src/main` 挪到 `src/default`): `merge.directoryRenames=true`, 跟着挪;
- `res/values*/strings.xml`: 按 key 三方合并 (`scripts/rebase/merge-android-strings.py`), 相邻的行各改各的不算冲突;
- fork 删掉的文件上游又改了: 保持删除;
- `scripts/rebase/take-fork.txt` 里 fork 整体重写过的文件: 整份取 fork 版;
- `.kt` 里每个冲突块都只是「两侧各自新增」或「两侧只动 import」: 取并集 (`scripts/rebase/union_resolve.py`)。

合并驱动与属性装在本地 (`.git/config`、`.git/info/attributes`), 子命令会自动装。改了规则跑
`scripts/rebase/test_*` (两个 Python 单测 + 一个在临时仓库里真跑 rebase 的集成测试)。

- `git rerere` 已启用 (`rerere.enabled` + `rerere.autoupdate`),重复冲突自动重放上次解法。
- 新增文案写进 `strings_izuko.xml`; tv 形态的源码在 `app/android/src/izukoTv` (上游 TV 客户端的
  tv flavor 占着 `src/tv`)。两者都是为了不和上游撞。
