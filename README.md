...
> Release 里默认下载 `universal`（包含全部架构，体积最大），电视、盒子、手机和平板都用它。个别盒子会拒装 `universal`（如腾讯极光盒子 7S），装不上时改下 `armeabi-v7a`，还不行再试 `arm64-v8a`。只有确切知道设备系统支持的架构时，才直接下对应的分架构包（`arm64-v8a` / `armeabi-v7a` / `x86_64`），体积更小——注意芯片或设备信息里显示 64 位，不代表能装 64 位的包，不少电视和盒子的系统是 32 位的，装 `arm64-v8a` 会失败。

### ⚠️ 系统版本要求

- **正式包最低 Android 8.1（API 27）**，更低的系统无法安装（报 `INSTALL_FAILED_OLDER_SDK`）。
- **Android 7.1（API 25）** 请下载文件名带 `legacy` 的兼容包（每个 Release 都附带，同样默认选 `legacy-universal`）。功能与正式包一致，但只在少量设备上验证过；7.1 上 BT 引擎跑在应用进程内，退到后台被系统回收后下载不会保活。
