...
//    }
    ...
    doLast {
        ...
        generatedSources.walkTopDown().filter { it.isFile && it.extension == "kt" }.forEach { file ->
            ...
            var text = original
            if (!text.startsWith("/*")) text = licenseHeader + "\n" + text
            // 有具体字段、又标了 `additionalProperties: true` 的对象 (v0 的 UserEpisodeCollection), 生成器会让它继承
            // `HashMap<String, Any>`: 只有 JVM 上能继承 (typealias 到 java.util.HashMap), 按多平台编 commonMain 元数据时
            // 它是 final, 编不过. 多出来的字段本来就由 ApiClient 的 ignoreUnknownKeys 丢掉, 不需要这个超类.
            text = text.replace(") : kotlin.collections.HashMap<String, kotlin.Any>() {", ") {")
...
val copyGeneratedToSrc = tasks.register("copyGeneratedToSrc", Sync::class) {
    ...
    // fixGeneratedOpenApi 是在生成器的输出上原地改的, Gradle 的文件快照跟不上, 会把这一步误判成 UP-TO-DATE
    // (源码目录里留着没修过的旧文件). 只在手动重新生成时跑, 每次都拷一遍即可
