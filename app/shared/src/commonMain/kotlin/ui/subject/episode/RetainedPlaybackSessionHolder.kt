...
class RetainedPlaybackSessionHolder : ViewModel(), PlaybackSessionEntry {
     ...
     * 给「播放页之外还要操作这个会话」的地方用: TV 的手机控制中心在播放器后台时从手机换源.
     * 只读; 会话的开关仍然只走本类自己的入口.
     */
...
private fun statusOf(
    ...
    // 地址已经交给播放器: 这时候播放器说的才是这一集的事. 排在选择层面的判断**前面**: 已经在播了就不存在
    // 「等你选数据源」—— 2026-09-12 真机上选中的资源在候选列表里一时对不上 (见 selectedMaybeExcludedMediaFlow),
    // 播得好好的会话被写成 NeedsSelection 一直挂着. 换集 / 换源时 loading 会先回到非 Succeed, 那时选择层面照常判
...
private fun problemOf(
    ...
    // 已经在播 (地址交给了播放器): 选择层面的问题不成立, 同 statusOf —— 否则后台会弹一句假的「请手动选择数据源」
    loading is VideoLoadingState.Succeed -> null
