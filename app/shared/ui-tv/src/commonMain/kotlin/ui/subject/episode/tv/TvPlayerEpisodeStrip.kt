 ...
 * 唤出, 平时完全不可见): 控制行隐藏 (调用方处理), 屏幕底部只剩一行标题 (见 [TvStripHeaderSlot])
 * 与卡片行 (一行正好 [TV_STRIP_VISIBLE_CARDS] 张完整卡, 聚焦卡左侧露上一张卡切边).
 * 点击卡片切换当前播放集并回纯画面; 再按下键进详情层.
 *
    ...
    BoxWithConstraints(
        ...
        Column(
            ...
            TvStripHeaderSlot(
                ...
                upNext = upNext,
                modifier = Modifier.padding(horizontal = TV_PLAYER_HORIZONTAL_PAD),
            )
 ...
 */
/**
 * 选集条顶上那一行标题: 平时是「剧集」, 倒计时态换成「接下来播放 + 还剩几秒」.
 *
 * **恒占一行, 两态只换字**: 整条选集条是贴底摆的 (见调用处的 align(BottomStart)), 这一行的高度
 * 只往卡片行上方长, 所以换字时卡片一动不动 —— 用户按方向键退出倒计时态时, 屏幕上唯一变化的
 * 就是这行字. 早先普通态不给这一行, 于是进出倒计时态时整条选集条会高一截矮一截.
 *
 * 两行叠在同一个 Box 里交叉淡入淡出, 而不是 if/else 直接换: 交替期间两者同时在场, 且都是单行
 * 同字号, Box 的高度自始至终不变.
 *
 * 单独拆成一个函数是必须的 (不能内联回上面那个 Column): [AnimatedVisibility] 有 ColumnScope
 * 重载, 在 Column 里写的话重载解析会挑中它, 而 Box 内容 lambda 里的隐式接收者是 BoxScope.
 */
@Composable
private fun TvStripHeaderSlot(
    countingDown: Boolean,
    upNext: TvUpNextState,
    modifier: Modifier = Modifier,
) {
    val motion = LocalAniMotionScheme.current.animatedVisibility
    Box(modifier) {
        AnimatedVisibility(
            visible = !countingDown,
            enter = motion.standardEnter,
            exit = motion.standardExit,
        ) {
            Text(
                stringResource(Lang.video_player_tv_episodes),
                color = Color.White,
