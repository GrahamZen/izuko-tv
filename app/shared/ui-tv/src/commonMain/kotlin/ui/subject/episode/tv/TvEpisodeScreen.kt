...
fun TvEpisodeScreenContent(
    ...
    // 预载条目详情 (TMDB 剧照/时长/简介, 详情层内容, 角色/制作人员): 详情层与选集条的增量信息共用同一 loader
    // (有"已加载"守卫, 不会重复请求). 分集列表本身不等它, 见 EpisodeViewModel.episodeListUiStateFlow.
    // 条目 id 必须用 vm.subjectId, 原因见 ensureTvSubjectDetails
    LaunchedEffect(Unit) {
    ...
    //
    // **播放意图 (playWhenReady) 也是键**: 暂停着就不计时; 从别处恢复播放 (手机遥控 / 一起看同步) 不经按键、
    ...
    LaunchedEffect(Unit) {
        combine(
        ...
        ) { _, playWhenReady -> playWhenReady }.collectLatest { playWhenReady ->
            ...
            // 暂停时不自动隐藏 (Prime 行为). 用 playWhenReady 而非严格 isPlaying: 卡在缓冲上不该把控制层永久钉住
            if (!playWhenReady) return@collectLatest
            ...
            Box(
                ...
                AniAnimatedVisibility(
                    visible = overlay.layer == TvPlayerLayer.HIDDEN && !skipTipVisible &&
                            !anySheetVisible && vm.videoScaffoldConfig.effectiveIdleProgressBarHeightDp > 0,
                    modifier = Modifier.align(Alignment.BottomStart),
                ) {
                    TvIdleProgressBar(
                        progressSliderState,
                        heightDp = vm.videoScaffoldConfig.effectiveIdleProgressBarHeightDp,
 ...
 *
 * 颜色与控制层的进度条同一套 (白 / 白 30%), 只是薄得多 (粗细由用户在设置里调, 见
 * `VideoScaffoldConfig.effectiveIdleProgressBarHeightDp`) —— 它不该被看成一个控件, 只是画面边缘的一道刻度.
 */
@Composable
