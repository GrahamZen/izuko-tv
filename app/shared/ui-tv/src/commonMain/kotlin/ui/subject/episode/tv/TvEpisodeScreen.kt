        ...
        AniTheme(darkModeOverride = DarkMode.DARK) {
            Box(
                ...
                AniAnimatedVisibility(
                    ...
                    TvIdleProgressBar(
                        progressSliderState,
                        heightDp = vm.videoScaffoldConfig.idleProgressBarHeightDp,
 ...
 * `VideoScaffoldConfig.idleProgressBarHeightDp`) —— 它不该被看成一个控件, 只是画面边缘的一道刻度.
 */
@Composable
 ...
 * | 双击中央 | 播放/暂停 (= 播放键) | 同左 |
 * | 双击左/右三分之一 | 退/进 [TV_PLAYER_SEEK_STEP_MILLIS] (= 单按左右键) | 同左 |
    ...
    Box(
        modifier.pointerInput(Unit) {
            awaitEachGesture {
                ...
                h.onDoubleTap(
                    when {
                        second.position.x < width / 3f -> TvTouchZone.LEFT
                        second.position.x > width * 2f / 3f -> TvTouchZone.RIGHT
