...
.cl-top.cv .cl-title, .cl-top.cv > .cl-meta { max-width: 64%; }
...
(function () {
  ...
  document.addEventListener('pointerdown', function (e) {
    ...
    if (b) b.parentNode.classList.add('press');
  });
  ...
  function renderResults(s) {
    ...
    resHead.innerHTML = '<div class="res-q">' + (s.keywords ? '「' + esc(s.keywords) + '」' : '筛选结果') + '</div>' +
      ...
      (s.filters ? ' · ' + esc(s.filters) : '') + '</span><span class="res-tip">点右边封面（或 ▶）直接播放，点其他地方看详情</span></div>';
    if (!items.length) {
      ...
      patchList(resList, items.map(function (x) {
        ...
        return '<div class="item res-item' + (x.cover ? ' cv' : '') + (x.blur ? ' nsfw-blur' : '') + '" data-sid="' + x.id + '">' +
  ...
  function groupHtml(g, s) {
    var h = '<h2>' + esc(g.name) + ' <small>' + g.total + ' 条</small></h2><div class="list">';
...
(function () {
    ...
    if (!d.ok) {
      ...
      d.groups.forEach(function (g) {
        ...
        h += '<div class="card cl-group' + (g.cover ? ' amb" data-art="' + art : '') + '">' +
          ...
          '<div class="cl-top' + (g.cover ? ' cv" data-art="' + art : '') + '">' +
          (g.cover ? window.coverLayers(g.cover) : '') +
          '<div class="cl-head"><div class="cl-title"' + (g.id ? ' data-open="' + g.id + '"' : '') + '>' + esc(g.title) + '</div>' +
          // 番名右边: 「缓存」小胶囊 (同播放卡, 点开这部番的缓存面板) + 整部删除 (只放垃圾桶图标, 先确认; 里面有正在播的那一集时确认框多说一句)
          (g.id ? '<button type="button" class="now-cache cl-cache" data-cache="' + g.id + '" data-title="' + esc(g.title) +
            '" aria-label="缓存更多剧集">' + window.ICONS.download + '缓存</button>' +
            '<button type="button" class="cl-delall icb" data-cdelall="' + g.id + '" data-title="' + esc(g.title) +
            '" data-count="' + g.items.length + '"' + (playingItem ? ' data-playing="' + esc(playingItem.label) + '"' : '') +
            ' aria-label="删除这部番的全部缓存" title="删除全部">' + window.ICONS.trash + '</button>' : '') +
          '</div><div class="cl-meta">' + esc(g.meta) + '</div></div>';
  ...
  function item(g, x) {
    ...
    // 每一集右边的暂停 / 继续 / 删除: 行行都有的通用动作, 只放图标 (删除仍是红底, 点了先确认)
    var I = window.ICONS;
    ...
    return '<div class="cl-ep" data-play="' + esc(x.cid) + '"><div class="n"><span class="cl-go">' + I.play + '</span>' + esc(x.label) +
      ...
      '<button type="button" class="cl-del icb" data-cact="delete" data-cid="' + esc(x.cid) + '" data-label="' + esc(g.title + ' ' + x.label) + '"' +
      (x.packShare ? ' data-share="' + x.packShare + '"' : '') + (x.playing ? ' data-playing="1"' : '') +
      ' aria-label="删除这一集" title="删除这一集">' + I.trash + '</button></div>';
  }
...
(function () {
  ...
  // 点头像 / 名字展开的账号菜单 (修改昵称 / 退出登录); nick = 正在改昵称
  ...
  document.addEventListener('click', function (e) {
    ...
    if (e.target.closest('#set-account [data-acct="menu"]')) { menu = !menu; nick = false; rerender(); return; }
...
(function () {
  ...
  function paintList() {
    ...
    hb.innerHTML = '<p class="hint">点右边封面（或 ▶）在电视上接着看，看完的播下一集；点其他地方打开详情页。</p><div class="list">' + items.map(function (x) {
      var imgs = x.imgs || [];
      return '<div class="item hist-item' + (imgs.length ? ' cv' : '') + '" data-sid="' + x.id + '">' + (imgs.length ? window.coverLayers(imgs) : '') +
