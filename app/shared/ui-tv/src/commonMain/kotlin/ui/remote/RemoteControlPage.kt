...
(function () {
  ...
  var noticeSeq = null, noticeFails = 0, noticeBusy = false, noticeSkip = 0;
  ...
  function pollNotice() {
    ...
    fetchT('api/notice' + (noticeSeq == null ? '' : '?after=' + noticeSeq), null, 6000)
      ...
      .catch(function (e) {
        ...
        if (++noticeFails >= 2) setTvState('off', '连不上电视：请确认电视开着、Ani 在运行，并且和手机在同一个网络');
...
(function () {
  ...
  function item(g, x) {
    ...
    return window.swRow('', del, '<div class="cl-ep sw-row" data-play="' + esc(x.cid) + '"><div class="n"><span class="cl-go">' + I.play + '</span>' + esc(x.label) +
...
(function () {
  ...
  var CACHE = sec('缓存', [
    ...
    '点一集：电视上播这一集；点番名：电视打开详情页。',
    '左滑一集：删除这一集（先确认）；暂停 / 继续用行右边的按钮。',
  ...
  var GENERAL = sec('账号', [
    ...
    '外观只影响这台手机；代理、BT Tracker、弹幕屏蔽词改完立即生效；最底下可以下载电视的日志。'
  ]);
  var SOURCES = sec('数据源', [
