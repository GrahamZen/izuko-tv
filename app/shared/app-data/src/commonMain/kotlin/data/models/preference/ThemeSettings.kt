 ...
 *
 * 按**开销的性质**分, 不是按"好看的程度":
 * - 按一下动一次的转场 (进详情页背景放大、追番页换分类卡片滑动、hero 文字分行错落): 只在操作那一刻花, 看得见,
 *   性能好的机器值得开 —— 均衡档起;
...
enum class TvVisualEffectsLevel {
    /** 流畅: 进详情页交叉淡入、换分类渐隐渐现、hero 文字整块进场; 没有常驻装饰; 不用原图. */
    Smooth,

    /** 均衡: 在流畅之上开转场 (背景放大 / 卡片滑动 / 文字错落). */
    Balanced,
...
data class ThemeSettings(
     ...
     *
     * **`null` = 还没显式选过**, 这时按老开关 [tvFullVisualEffects] 推导 (开过 = 完整, 否则流畅) —— 读取一律走
