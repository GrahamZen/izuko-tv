...
internal class PlayerLoadErrorHandler(
    ...
    fun blacklist(mediaId: String) {
        if (mediaId in blacklistedMediaIds) return
        blacklistedMediaIds = blacklistedMediaIds.add(mediaId)
        // 这张表整个会话只增不减, 拉错一条就少一个能用的源 —— 出问题时要能从日志看出是谁拉黑的
        logger.info { "Blacklisted media $mediaId, ${blacklistedMediaIds.size} blacklisted in this session" }
    }
...
/** [isPlayerLifecycleError] 找 cause 链的深度上限: 链可能自引用. */
private const val MAX_CAUSE_DEPTH = 8

/**
 * 是不是**播放器生命周期错误**: 离开播放页时释放 Surface 超时 (`Detaching surface timed out`) 这类,
 * 跟这条源能不能播毫无关系 —— 缓存全速下载时 IO 与主线程都忙, Surface 释放最容易超时.
 *
 * 当成"源坏了"去换源, 代价是双份的: 当场中断播放并重新搜索 (那时用户已经离开播放页, 换了也没人看),
 * 还把这条好源拉进**只增不减**的黑名单, 之后整个会话的自动选源都挑不到它 —— 表现就是
 * "一边下载一边就用不了在线源了".
 *
 * 按类名与消息匹配而不是 catch 具体类型: 这里是 commonMain, 看不见 ExoPlayer 的异常类.
 */
internal fun PlayerLoadError.isPlayerLifecycleError(): Boolean {
    var current: Throwable? = cause
    var depth = 0
    while (current != null && depth++ < MAX_CAUSE_DEPTH) {
        val name = current::class.simpleName.orEmpty()
        val message = current.message.orEmpty()
        if (name.contains("ExoTimeout") ||
            message.contains("Detaching surface", ignoreCase = true) ||
            message.contains("surfaceDestroyed", ignoreCase = true)
        ) {
            return true
        }
